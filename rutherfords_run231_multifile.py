...
from data_index_constants_run231 import IndexConstants
...
# file to read:
file_array = array(["F:/fiona/run231/r231f036_4844.lmd", "F:/fiona/run231/r231f036_4845.lmd"])


## destination file:
output_file = open('F:/fiona/run231/rutherfords/r231f036_ruthe.txt', 'w')
#output_file = open('F:/fiona/run221/data/r221f025_Es_DS.txt', 'w')
#t1 = time.time()
...
def analyze_event(f):
    ...
    for part in temp_event:
        try:
            ...
            # elif index_name.startswith('cave_1_x_strip'):
                # detector = "C1"
                # x = index_name.split('_')[-1]
                # back = part.data
                # all_x.append(x)
                # all_back.append(back)
            # if index_name.startswith('cave_2_front_strip'):
                # detector = "C2"
    ...
    if (IndexConstants.clock_low.name in temp_event_dict):
        ...
        if detector == "ruthe" and ruthe > 1000:
            f.write(f"{file_name[-17:]}\t{detector}\t{seconds}\t{current_event_number}\t{ruthe}\n")
